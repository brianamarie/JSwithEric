...
JsKoansReporter.prototype.reportRunnerStarting = function(runner) {
  this.outerDiv = this.createDom('div', { className: 'jasmine_reporter show-passed' },
      this.createDom('h2', { }, "Javascript: Working with Helper Functions"),
...
JsKoansReporter.prototype.reportRunnerResults = function(runner) {
  ...
  if (this.failedSpecs === 0) {
    status = 'passed';
    enlightenmentMessage = "All done! Nice work!";
  } else {
    status = 'failed';
    enlightenmentMessage = "You aren't done yet, keep at it!";
  }
  ...
  var progress = this.createDom('div', {}, 
      ...
      this.createDom('div', { className: 'completion' }, 
        ...
        this.createDom('div', {},
          this.createDom('span', { className: 'key' }, "Tests Complete: "),
          this.createDom('span', { className: 'value' }, specsCount - this.failedSpecs + "/" + runner.specs().length)
          ),
        this.createDom('div', { className: 'options' },
          this.createDom('label', { "for": "__jsKoans_showPassed__" }, " Show Completed Challenges"),
...
JsKoansReporter.prototype.reportSpecResults = function(spec) {
  ...
  if ( !results.passed() ) {
    ...
    description = "Test: (" + spec.description + ") is not yet correct."
  } else {
    description = "Test:  '" + spec.description + "' was solved!"
  }

  var specDiv = this.createDom('div', { className: 'spec '  + skipStatus },
      this.createDom('a', { className: 'run_spec_' + status, href: '?spec=' + encodeURIComponent(spec.getFullName()) }, "refresh"),
      this.createDom('a', {
  ...
  for (var i = 0; i < resultItems.length; i++) {
    ...
    if (result.type == 'expect' && result.passed && !result.passed()) {
      ...
      messagesDiv.appendChild(
          this.createDom('div', { className: 'description' }, "Look here for clues:")
          );
